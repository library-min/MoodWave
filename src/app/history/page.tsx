...
export default function HistoryPage() {
  ...
  const emotionStats = monthEntries.reduce((acc, curr) => {
    acc[curr.emotion] = (acc[curr.emotion] || 0) + 1
    return acc
  }, {} as Record<string, number>)
  ...
  if (!user) return (
    <div style={{ minHeight: '100vh', background: '#0d0d14', color: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20 }}>
  ...
  return (
    <main className="mw-history-page">
      <style>{`
        ...
        .detail-track { display: flex; align-items: center; gap: 16px; padding: 12px; border-radius: 20px; background: rgba(255,255,255,0.03); border: 1px solid transparent; color: inherit; transition: all 0.3s ease; cursor: pointer; position: relative; overflow: hidden; }
          ...
          <div className="calendar-section">
            <div className="calendar-wrap">
              <div className="cal-header">
                <button onClick={handlePrevMonth} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer' }}>&larr;</button>
                <div style={{ fontFamily: 'DM Serif Display', fontSize: 24 }}>{currentMonth.getFullYear()} / {currentMonth.getMonth() + 1}</div>
                <button onClick={handleNextMonth} style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer' }}>&rarr;</button>
              </div>
              <div className="cal-grid">
                {['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'].map(d => (
                  <div key={d} style={{ textAlign: 'center', fontSize: 11, opacity: 0.2, marginBottom: 12 }}>{d}</div>
                ))}
                {calendarDays.map((day, i) => {
                  ...
                  return (
                    <div key={dateStr} className={`cal-day ${topEmotion ? 'has-record' : ''} ${selectedDate === dateStr ? 'selected' : ''} ${isToday ? 'today' : ''}`} onClick={() => handleDateClick(dateStr)}>
                      <span>{day.getDate()}</span>
                      {topEmotion && <span style={{ fontSize: 18, marginTop: 4 }}>{emotionEmojis[topEmotion] || '🎵'}</span>}
                    </div>
                  )
                })}
              </div>
              ...
              {sortedStats.map(([emotion, count]) => (
                <div key={emotion} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                  ...
                  <div style={{ flex: 1, height: 4, background: 'rgba(255,255,255,0.05)', borderRadius: 10, overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${(count / monthEntries.length) * 100}%`, background: 'linear-gradient(90deg, #c4a8ff, #8ab4ff)' }} />
                  </div>
                  ...
                  <div className="fade-up" key={selectedEntry.id}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 32 }}>
                      {selectedEntry.keywords.map(k => (
                        <span key={k} style={{ fontSize: 11, padding: '4px 10px', borderRadius: 100, border: '1px solid rgba(196,168,255,0.2)', color: '#c4a8ff' }}>#{k}</span>
                      ))}
                    </div>
        ...
        <div className="mw-mini-player fade-up">
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1, minWidth: 0 }}>
            <div style={{ minWidth: 0 }}>
              ...
              <div style={{ fontSize: 12, opacity: 0.4 }}>{formatTime(currentTime)} / {formatTime(duration)}</div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginRight: 20 }}>
            <span style={{ fontSize: 12, opacity: 0.5 }}>Vol</span>
            <input type="range" min="0" max="1" step="0.01" value={volume} onChange={handleVolumeChange} style={{ width: 80, accentColor: '#c4a8ff', cursor: 'pointer' }} />
          </div>
