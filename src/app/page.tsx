...
export default function MoodWaveApp() {
  ...
  // ✅ 제거: const supabase = createClient() 여기 없음
    ...
    <div className="mw-app">
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
        ...
        .mw-login-btn { background: rgba(255,255,255,0.06); border: 1px solid var(--glass-border); color: white; padding: 10px 18px; border-radius: 100px; cursor: pointer; font-size: 13px; backdrop-filter: blur(10px); font-family: 'Noto Serif KR', serif; }
        .mw-user-avatar { width: 36px; height: 36px; border-radius: 50%; background: var(--accent-start); color: #0d0d14; display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 13px; }
        ...
        .mw-input { width: 100%; background: transparent; border: none; border-bottom: 1px solid rgba(255,255,255,0.15); padding: 20px 0; font-size: 22px; color: var(--text-main); outline: none; transition: border-color 0.4s ease; border-radius: 0; font-family: 'Noto Serif KR', serif; }
        .mw-input:focus { border-bottom-color: var(--accent-start); }
        .mw-input::placeholder { color: rgba(255,255,255,0.15); }
        .mw-btn-analyze { width: 100%; padding: 22px; border-radius: 100px; border: none; background: linear-gradient(135deg, var(--accent-start), var(--accent-end)); color: #0d0d14; font-size: 17px; font-weight: 700; cursor: pointer; transition: all 0.3s; display: flex; align-items: center; justify-content: center; gap: 10px; font-family: 'Noto Serif KR', serif; }
        .mw-btn-analyze:hover:not(:disabled) { transform: translateY(-3px); box-shadow: 0 15px 40px rgba(196,168,255,0.4); }
        .mw-btn-analyze:disabled { opacity: 0.5; cursor: not-allowed; }
        ...
        .mw-country-tab { padding: 10px 20px; border-radius: 100px; border: 1px solid var(--glass-border); background: transparent; color: var(--text-sub); font-size: 13px; cursor: pointer; white-space: nowrap; font-family: 'Noto Serif KR', serif; }
        ...
        .mw-track:hover { background: rgba(255,255,255,0.06); }
        .mw-track.playing { background: rgba(196,168,255,0.12); }
        .mw-album-art { width: 56px; height: 56px; border-radius: 12px; flex-shrink: 0; object-fit: cover; }
        ...
        .mw-player-seek-bar { height: 100%; background: var(--accent-start); transition: width 0.1s linear; }
        ...
        .mw-toast { position: fixed; bottom: 100px; left: 50%; transform: translateX(-50%); background: rgba(255,255,255,0.1); backdrop-filter: blur(20px); color: white; padding: 12px 24px; border-radius: 100px; font-size: 14px; border: 1px solid rgba(255,255,255,0.1); z-index: 2000; white-space: nowrap; }
      ...
      <div className="mw-background">
        <div className="mw-glow-layer">
          <div className="mw-orb orb-1" style={{ background: `radial-gradient(circle, ${currentColors[0]}26 0%, transparent 70%)` }} />
          <div className="mw-orb orb-2" style={{ background: `radial-gradient(circle, ${currentColors[1]}26 0%, transparent 70%)` }} />
          <div className="mw-orb orb-3" style={{ background: `radial-gradient(circle, ${currentColors[2]}26 0%, transparent 70%)` }} />
        </div>
      </div>

      <div className="mw-auth-bar">
        {user ? (
          <>
            <div className="mw-user-avatar">{user.email?.charAt(0).toUpperCase()}</div>
            <button className="mw-login-btn" onClick={handleLogout}>로그아웃</button>
          </>
        ) : (
          <button className="mw-login-btn" onClick={handleGoogleLogin}>구글로 시작</button>
        )}
      </div>
      ...
      <div className="mw-container">
        {view === 'home' && (
          <div className="fade-up">
            ...
            <div className="mw-input-wrap">
              <input className="mw-input" value={text} onChange={(e) => setText(e.target.value)} placeholder="예: 고요한 새벽, 혼자만의 시간" onKeyDown={(e) => e.key === 'Enter' && handleAnalyze()} autoFocus />
            </div>
        ...
        {view === 'loading' && (
          <div className="mw-loading-view fade-up">
            ...
            <div className="mw-loading-card">
              <div style={{ display: 'flex', gap: '10px', height: '100px', alignItems: 'center' }}>
                {[...Array(9)].map((_, i) => (
                  <div key={i} className="mw-bar" style={{ height: `${[40, 70, 50, 90, 60, 80, 45, 65, 40][i]}px`, animationDelay: `${i * 0.1}s` }} />
                ))}
              </div>
            </div>
        ...
        {view === 'result' && result && (
          <div className="fade-up">
            <button onClick={reset} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid var(--glass-border)', color: 'white', padding: '10px 20px', borderRadius: '100px', cursor: 'pointer', marginBottom: '40px', fontFamily: 'Noto Serif KR, serif' }}>←</button>
            ...
            <div className="mw-emotion-card">
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '16px', marginBottom: '20px', flexWrap: 'wrap' }}>
                <span style={{ fontSize: '32px', fontWeight: '700' }}>{result.mood.emotion}</span>
                <span style={{ fontSize: '15px', color: 'var(--accent-start)' }}>#{result.mood.keywords.join(' #')}</span>
              </div>
              <p style={{ fontSize: '17px', lineHeight: '1.8', color: 'rgba(240,238,232,0.8)', fontWeight: '300' }}>{result.mood.summary}</p>
            </div>

            <div className="mw-country-tabs">
              {(['KR', 'JP', 'US'] as Country[]).map((c) => (
                <button key={c} className={`mw-country-tab ${selectedCountry === c ? 'active' : ''}`} onClick={() => handleCountryChange(c)}>
                  {c === 'KR' ? '🇰🇷 한국' : c === 'JP' ? '🇯🇵 일본' : '🌍 글로벌'}
                </button>
              ))}
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
              <h3 style={{ fontSize: '13px', letterSpacing: '0.2em', color: 'var(--text-sub)', textTransform: 'uppercase' }}>Recommended Playlist</h3>
              <button onClick={handleShare} disabled={sharing} style={{ background: 'transparent', border: `1px solid ${copied ? '#10b981' : 'var(--accent-start)'}`, color: copied ? '#10b981' : 'var(--accent-start)', padding: '8px 16px', borderRadius: '100px', cursor: 'pointer', fontSize: '12px', fontFamily: 'Noto Serif KR, serif' }}>
                {sharing ? '생성 중...' : copied ? '복사됨 ✓' : '🔗 공유하기'}
              </button>
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {loadingTracks ? (
                [...Array(4)].map((_, i) => <div key={i} style={{ height: 80, background: 'rgba(255,255,255,0.03)', borderRadius: 20 }} />)
              ) : (
                result.tracks.map((track, idx) => (
                  <div key={track.id} className={`mw-track ${playingId === track.id ? 'playing' : ''}`} onClick={() => handlePlay(idx)}>
                    <img src={track.albumImage || ''} className="mw-album-art" alt="" />
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div className="mw-player-title" style={{ fontSize: 15 }}>{track.name}</div>
                      <div className="mw-player-artist" style={{ fontSize: 13 }}>{track.artist} · {track.album}</div>
                    </div>
                    <div style={{ fontSize: 14 }}>{playingId === track.id && !isPaused ? '■' : '▶'}</div>
                    {playingId === track.id && <div className="mw-track-progress" style={{ width: `${(currentTime / duration) * 100}%` }} />}
                  </div>
                ))
              )}
            </div>
      ...
      {currentTrack && (
        <div className="mw-player">
          ...
          <div className="mw-player-info">
            <img src={currentTrack.albumImage || ''} style={{ width: 48, height: 48, borderRadius: 8, objectFit: 'cover' }} alt="" />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginRight: 16 }}>
            <span style={{ fontSize: 10, opacity: 0.4 }}>Vol</span>
            <input type="range" min="0" max="1" step="0.01" value={volume} onChange={handleVolumeChange} style={{ width: 60, accentColor: '#c4a8ff', cursor: 'pointer' }} />
          </div>
          ...
          <div style={{ fontSize: 12, opacity: 0.5, minWidth: 80, textAlign: 'center', fontFamily: 'monospace' }}>
